...
export class AddToCartComponent implements OnInit {
    ...
    displayHeader = false;
    cartItem:any;
    constructor( @Inject(MAT_DIALOG_DATA) public data: any, private activityhub: ActivityHubService, private _formBuilder: UntypedFormBuilder, private route: ActivatedRoute, private dialogRef: MatDialogRef<AddToCartComponent>, private cartservice: CartService, private dataservice: DataService) { }
    ...
    get participants(): UntypedFormArray {return this.AtcForm.get('participants') as UntypedFormArray; }
    ...
    selectParticipant(event) {
        const formArray: UntypedFormArray = this.AtcForm.get('participants') as UntypedFormArray;
        if(event.checked){
            ...
            this.participants.controls.forEach((ctrl: UntypedFormControl) => {
    ...
    onaddtlChange(event) {
        const formArray: UntypedFormArray = this.AtcForm.get('additionalcharges') as UntypedFormArray;
        //console.log(event, ' ', event.checked,' ', event.source.value);
        /* Selected */
        if(event.checked){
            // Add a new control in the arrayForm
            formArray.push(new UntypedFormControl(event.source.value, [Validators.required]));
    ...
    createParticipants(): UntypedFormGroup {
